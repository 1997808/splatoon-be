...
export class Migration20240703035153 extends Migration {
  async up(): Promise<void> {
    this.addSql(
      'create table "category" ("id" serial primary key, "name" varchar(255) not null, "is_highlight" boolean not null);',
    );
    this.addSql(
      'create table "transaction" ("id" serial primary key, "item" varchar(255) not null, "description" text null, "amount" numeric(15,2) not null, "type" text check ("type" in (\'income\', \'expense\')) not null, "user_id" int not null, "category_id" int not null, "transaction_date" date not null);',
    );
    this.addSql(
